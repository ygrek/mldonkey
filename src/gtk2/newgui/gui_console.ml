  ...
  object (self)
    ...
    method insert_line t =
      if Fifo.length lines > !max_console_lines + line_threshold then begin
          ...
          text#buffer#delete ~start: (text#buffer#get_iter_at_char line_pos) ~stop: (text#buffer#get_iter_at_char (line_pos+len));
          removed_size <- removed_size + len;
        end;
      let len = String.length t in
      Fifo.put lines (current_pos, len);
      current_pos <- current_pos + len;
      ignore (text#buffer#insert t ~iter: text#buffer#end_iter (* ~pos:0 *) );
...
(* GTK2      text#thaw (); *)
(* GTK2      text#set_position text#length *)
      ()
...
(*              we_command#set_item_string (GList.list_item ~label:s ()) s;*)
    ...
    initializer
      (try 
          ...
          text#misc#set_style style;
        with _ -> ());
(* GTK2      we_command#set_use_arrows `ALWAYS; *)
