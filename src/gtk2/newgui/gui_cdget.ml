...
class ['a] tree (selection: 
  ...
  let item = GBroken.tree_item ~label: label () in
  let wtree = GBroken.tree () in
  let subtrees = ref [] in
  let datas = ref [] in
  object(self)
...
lprint_newline (); *)
        let wtree = try
          ...
          with _ ->
              let item = GBroken.tree_item ~label: artist () in
              wtree#append item;
              let wt_sub = GBroken.tree () in
...
(* GTK2        label#set_alignment ~x: 0.0 ~y: 0.5 (); *)
        let item = GBroken.tree_item () in
