...
(* GTK2 ~foreground: (Gui_misc.color_of_name name) *) 
      name;
...
(* GTK2      wt_dialog#set_position (wt_dialog#length - 1) *)
    ...
    initializer
    let return () = 
      let s = wt_input#buffer#get_text () (* get_chars 0 wt_input#length *) in
      let len = String.length s in
      let s2 = 
        if len <= 0 then s
        else
      ...
      self#send s2;
      wt_dialog#buffer#insert 
(* GTK2 ~foreground: (Gui_misc.color_of_name !Gui_options.client_name) *)
...
class box columns friend_tab =
  ...
  object (self)
    inherit [gui_client_info] Gpattern.filtered_plist `MULTIPLE titles true (fun c -> c.gclient_num) as pl
...
class pane_friends () =
  ...
  let wnote_chat =
    GPack.notebook ~homogeneous_tabs:false ~show_border:true
      ~scrollable:true (* GTK2 ~popup:false *) ()
  in
  object (self)
