...
let save_gui_options gui =
  ...
  (
   ...
   | w :: _ ->
       ...
       let (w,h) = Gdk.Drawable.get_size w#misc#window in
       O.gui_width =:= w;
       O.gui_height =:= h
  );
...
let set_hpaned (hpaned : GPack.paned) prop =
  let (w1,_) = Gdk.Drawable.get_size hpaned#misc#window in
  let ndx1 = (w1 * !!prop) / 100 in
  hpaned#child1#misc#set_size_request ~width: ndx1 ()

let set_vpaned (hpaned : GPack.paned) prop =
  let (_,h1) = Gdk.Drawable.get_size hpaned#misc#window in
  let ndy1 = (h1 * !!prop) / 100 in
  hpaned#child1#misc#set_size_request ~height: ndy1 ()
...
let get_hpaned gui (hpaned: GPack.paned) prop =
  ...
  ignore (hpaned#child1#coerce#misc#connect#size_allocate
      ~callback: (fun r ->
        let (w1,_) = Gdk.Drawable.get_size hpaned#misc#window in
        prop =:= r.Gtk.width * 100 / (max 1 (w1 (* GTK2 - hpaned#handle_size *) ));
...
let get_vpaned gui (hpaned: GPack.paned) prop =
  ...
  ignore (hpaned#child1#coerce#misc#connect#size_allocate
      ~callback: (fun r ->
        let (_,h1) = Gdk.Drawable.get_size hpaned#misc#window in
        prop =:= r.Gtk.height * 100 / (max 1 (h1 (* GTK2 - hpaned#handle_size *)));
        save_gui_options gui
    ))
