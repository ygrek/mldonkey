                ...
                let rec iter n l = 
                  ...
                  | (Pixmap p) :: q -> 
                      (try 
                        ...
                        with 
                          e -> lprintf "Exception %s in gpattern#update_row Pixmap\n"
                  ...
                  | (Pixtext (s, p)) :: q ->
                      (try 
                        ...
                        with 
                          e -> lprintf "Exception %s in gpattern#update_row Pixtext\n"
                              (Printexc2.to_string e));
                      iter (n+1) q
                in
                iter 0 l;
(*                tooltips#set_tip ((wlist:int)#get_row row)#coerce ~text: "Tooltip";*)
                ...
                | Some c -> 
                    (try 
                      ...
                      with 
                        e -> lprintf "Exception %s in gpattern#update_row col_opt\n"
                            (Printexc2.to_string e));
              end
          with e ->
...
(* connect the press on button 3 for title contextual menu *)
      for col = 0 to wlist#columns - 1 do
        ...
        let b = Widget.get_ancestor w#as_widget (Gobject.Type.from_name "GtkButton") in
        let button = new GButton.button (Gobject.unsafe_cast b) in
