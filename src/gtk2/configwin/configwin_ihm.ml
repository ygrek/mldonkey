...
(* GTK2      ~ok_if_empty: param.combo_blank_allowed  *)
...
class color_param_box param =
  ...
  let f_sel () =
	...
	(fun () -> 
	  ...
	  let r = Gdk.Color.red color * 255 in
	  let g = Gdk.Color.green color * 255 in
	  let b = Gdk.Color.blue color * 255 in
...
class font_param_box param =
  ...
  let _ = 
	...
	tooltips#set_tip wb#coerce ~text: help ~privat: help 
  in
  let set_entry_font s = (* GTK2 font_opt =
   match font_opt with
      None -> ()
    | Some s -> *)
  ...
  let f_sel () =
	...
	(fun () -> 
	  let font_opt = dialog#selection#font_name in
          we#set_text (* GTK2 match font_opt with None -> "" | Some s -> s) *)
          font_opt;
