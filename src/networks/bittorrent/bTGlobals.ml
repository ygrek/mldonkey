...
let new_file file_shared file_id =
  ...
  let partition = fixed_partition file_shared.file_swarmer
  ...
  Int64Swarmer.set_verifier partition (fun b ->
      if file_info.file_info_chunks = [||] then raise Not_found;
...
let new_client file peer_id kind =
  ...
  with _ ->
      ...
      file_add_source (as_file file.file_shared.file_file) (as_client c);
      c
