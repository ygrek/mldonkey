...
(* Don't share files greater than 10 MB on Gnutella2 and limit to 200 files. 
...
that we can reuse queries *)
      ...
      Gnutella2.send_query uid words xml_query;
  ...
  network.op_network_share <- (fun fullname codedname size ->
      if !shared_files_counter < max_shared_files &&
        size < max_shared_file_size then begin
          ...
          Gnutella2Protocol.new_shared_words := true;
...
let _ =
  result_ops.op_result_download <- (fun result _ force ->
      Gnutella2Servers.download_file result)
...
let _ =
  ...
  file_ops.op_download_recover <- (fun file ->
      Gnutella2Servers.recover_file file;
      List.iter (fun c ->
          Gnutella2Servers.get_file_from_source c file
      ) file.file_clients
  );
...
(fun s -> Gnutella2Servers.disconnect_server s BasicSocket.Closed_by_user);
...
(* Start a download for this file *)
              ...
              Gnutella2Servers.download_file r;
              true
            end
          else false
  )
...
let browse_client c = 
  lprintf "Gnutella2: browse client not implemented\n";
  ()
...
let commands = [
    "g2stats", Arg_none (fun o ->
        ...
        ""
    ), " :\t\t\t\tprint stats on Gnutella2 network";
