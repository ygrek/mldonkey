...
module Gnutella2Handler = struct
    let init s sock gconn =
      Gnutella2Handler.init s sock gconn
  end
...
*)
    ...
    s.server_connected <- int32_time ();
    if !deflate then
      s.server_sock <- CanBeCompressed.deflate_connection sock;
    Gnutella2Handler.init s sock gconn 
...
and connect_server h headers =  
  ...
  | NoConnection -> 
      ...
      Gnutella2Proto.server_send_ping s.server_sock s;
...
(* Start by Gnutella2 headers *)
                  ...
                  Printf.bprintf buf "Listen-IP: %s:%d\r\n"
                    (Ip.to_string (client_ip s.server_sock)) !!client_port;
...
(* do as if connection failed. If it connects, connection will be set to OK *)
          ...
          List.iter (fun s ->
              Gnutella2Proto.server_send_push s uid uri
          ) !g2_connected_servers;
      | _ ->
          Gnutella2Clients.connect_client c
    end
...
let disconnect_server s r =
  match s.server_sock with
  | Connection sock | CompressedConnection (_,_,_,sock) -> close sock r
...
let udp_handler p =
  ...
  if String.sub buf 0 3 = "GND" then
    Gnutella2.udp_handler ip port buf
  else
