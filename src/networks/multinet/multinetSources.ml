...
Pour chaque fichier, on doit posseder la liste complete des sources, avec
  ou non un client associe. A partir d'une source, il doit etre possible
  de trouver facilement le client associe s'il existe.

Ordonancement:
 * Prendre la liste des fichiers, triee dans l'ordre des priorites 
    decroissantes: pour chaque priorite, conserver la liste des fichiers
    ayant cette priorite.
 * Passer les fichiers qui ne sont pas dans l'etat "downloading"
 * Pour chaque priorite, diviser le nombre de connections possibles par le
     nombre de fichiers, puis prendre chaque fichier, et essayer ce nombre
     de connections. Repeter cette operation pour cette priorite tant qu'une
     connection a pu etre essayee, puis passer a la priorite suivante.

 * Pour chaque fichier, trier les sources dans les files suivantes:

     1) Excellentes sources: doivent etre recontacter des qu'une operation 
         est possible. Doivent etre triees toujours avec la premiere connection
         possible en tete.

     2) Nouvelles sources: sources annoncees, mais jamais connectees

     3) Bonnes sources: sources contactees, mais couramment non disponibles
         pour un transfert

     4) Mauvaises sources: sources pour lesquelles la derniere tentative
         de connection a echouer. A chaque echec, on double le temps de
         reconnection, avec une heure pour maximum.

Lorsqu'une connection est possible avec une source, elle est placee dans la
  file d'attente pour son reseau, et c'est le reseau qui doit ensuite
  la replacer dans la file d'entree des sources.
*)
...
let dummy_network = new_file_network "DUMMY"
  
let dummy_source = {
    source_num = 0;
    source_addr = Known_location (Ip.null, 0);
    source_files = [];
    source_requests = [];
    source_age = 0;
    source_network = dummy_network;
    source_retry = 0;
    source_score = 0;
  }
...
let has_source_request s file =
  try
    ignore (iter_has_request s.source_requests file); true
  with _ -> false
  
let find_source_request s file = 
  let files = s.source_requests in
...
let add_request s file time result =
  ...
  s.source_requests <- r :: s.source_requests

let set_source_score s score =
  s.source_score <- maxi 0 (mini score (sources_nqueues - 1))
  
let add_source_request s file time result =
  set_source_score s (s.source_score +
    match result with
    | File_possible -> 1
    | File_not_found -> 0
    | File_expected -> 2
...
let add_file_source file s = 
  if not (Intmap.mem s.source_num file.file_locations) then
    begin
      file.file_locations <- Intmap.add s.source_num s file.file_locations ;
      s.source_files <- file :: s.source_files;
      Queue.put file.file_sources.(s.source_score) (s.source_retry, s)
    end
      
let new_source addr network file = 
  let s =
    try
      H.find sources { dummy_source with source_addr = addr }
    with _ ->
        let s =
          {
            source_num = CommonClient.book_client_num ();
            source_addr = addr;
            source_network = network;
            source_files = [];
            source_requests = [];
            source_age = last_time ();
(* since new sources are sorted by retry, we cannot use 0 for retry *)
            source_retry = last_time () - 3600;
        ...
        s
  in
  add_file_source file s;
  s


let resort_files () = 
  let files = List.sort (fun f1 f2 ->
        file_priority f1 - file_priority f2
    ) !current_files in
  let rec iter files sorted prio rem =
    match files with
      [] ->
        if rem = [] then sorted else (prio, rem) :: sorted
    | file :: tail ->
        if file_priority file = prio then
          iter tail sorted prio (file :: rem)
        else
        let sorted = 
          if rem = [] then sorted else (prio, rem) :: sorted in
        iter tail sorted (file_priority file) [file]
  in
  sorted_files := List.map snd (iter files [] max_int [])
  
let minute_timer () = 
(* Every minute, decrease the score of all sources by 1. After 10 minutes,
all sources have a score of 0. But if the connection succeeds, the score
will increase. *)
  H.iter (fun s -> 
      s.source_score <- maxi 0 (s.source_score - 1)
  ) sources;
  
(* Sort the files by priority *)
  resort_files ()
...
let second_timer () = 
(* How to be fair: currently, the sources will always been connected in the
same order, so that a very popular file may prevent another file from being
downloaded. *)
  
  
(* For all files of the same priority, connect until no connection was 
  possible *)
  let iter_raw files max_conns =
    max_conns
  in
  
  
(* From the highest priority to the lowest *)
  let rec iter lists max_conns =
    match lists with
      [] -> ()
    | files :: tail ->
        let max_conns = iter_raw files max_conns in
        iter tail max_conns
  in
  iter !sorted_files !!max_clients_per_second
