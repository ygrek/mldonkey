...
* When consecutive ranges have been downloaded, we could merge them.
  ...
  ranges ?
* If there are different verification schemes available, we should mark
  each range as verified or not, so that corrupted blocks do not force
  redownload of overlapped ranges.
* The client_num should be given in received, and stored so that it can
  be suspected if a block is corrupted.
  *)
...
(* Only when bitmaps are used instead of ranges *)
          ...
          mutable part_ages : int array;
          mutable part_availability : int array;
        }
...
(* Set the block as completely and verified *)
      let loaded_block b =
        lprintf "-----> block correct... downloaded\n";
...
(* Set the block as completely empty, to be reloaded *)
      let reload_block b =
        lprintf "-----> block corrupted... reloading\n";
        let p = b.block_partition in
        let t = p.part_t in
...
(* OK, we just need to wait for more data *)
      ...
      let compute_bitmap p =
        ...
        and iter_ranges b r current =
          if r.range_begin >= b.block_end then begin
              ...
              if p.part_bitmap.[b.block_num] < '2' then begin
      ...
      let apply_partition t part = 
        ...
        if String.length part.part_bitmap <> part.part_nblocks then begin
            part.part_bitmap <- String.make part.part_nblocks '0';
            part.part_availability <- Array.create part.part_nblocks 0;
            part.part_ages <- Array.create part.part_nblocks 0;
            compute_bitmap part
          end
...
(* In the original eDonkey plugin, a signal is sent to update the file
  information if a threshold is reached between the new and old value. *)
                
                p.part_ages.(b.block_num) <- BasicSocket.last_time ();
                p.part_availability.(b.block_num) <- 
                  p.part_availability.(b.block_num) + 1;
...
start at the beginning of the range. *)
              ...
              if r.range_current_begin < file_end then
                if r.range_current_begin >= file_begin then
                  let new_range_current_begin = 
                    if r.range_end <= file_end then begin
                        t.t_dirty <- true;
                        r.range_end
                      end else file_end 
                  in
      ...
      let set_verified_bitmap p bitmap =
        let len = String.length bitmap in
        p.part_bitmap <- bitmap;
        p.part_availability <- Array.create len 0;
        p.part_ages <- Array.create len 0;
        let rec iter_blocks b =
          if p.part_bitmap.[b.block_num] = '3' then loaded_block b;
          match b.block_next with
            None -> ()
          | Some b -> iter_blocks b
        in
        iter_blocks p.part_blocks; 
        compute_bitmap p
      ...
      let recheck_partition p reverify =
        for i = 0 to String.length p.part_bitmap do
          if reverify || p.part_bitmap.[i] < '3' then
            p.part_bitmap.[i] <- '2'
        done;
        p.part_t.t_dirty <- true
...
(* 1: find completed blocks *)
        let rec iter_blocks b =
          if p.part_bitmap.[b.block_num] <= '2' then
            iter_ranges b b.block_ranges.range_verified b.block_ranges;
          next_block b
        
        and iter_ranges b verified r =
          if r.range_begin < b.block_end then begin
              if r.range_current_begin >= r.range_end then
                next_range b (verified && r.range_verified) r
            end
          else
            p.part_bitmap.[b.block_num] <- if verified then '3' else '2'
        
        and next_range b verified r =
          match r.range_next with
            None -> p.part_bitmap.[b.block_num] <- if verified then '3' else '2'
          | Some rr -> iter_ranges b verified rr 
...
(* 2: verify completed blocks *)
    ...
    let rec debug_ranges buf r =
      ...
      (Integer.to_string r.range_end)
        (if r.range_current_begin = r.range_end then 
            if r.range_verified then "[VERIFIED]" else "[DONE]" else
        if r.range_current_begin = r.range_begin then "" else
          Printf.sprintf "[%s]" (Integer.to_string 
              (r.range_end -- r.range_current_begin)));
      match r.range_next with
    ...
    let debug_print buf t =
      ...
      let block_contributors b = b.block_contributors
      let set_block_contributors b l = 
        b.block_contributors <- l
      let add_block_contributor b ip =
        if not (List.mem ip b.block_contributors) then
          b.block_contributors <- ip :: b.block_contributors

      let block_legacy b = b.block_legacy
      let set_block_legacy b v = b.block_legacy <- v

      let blocks_availability p = p.part_availability
      let blocks_age p = p.part_ages
        
(* Compute the age only on absent chunks *)
      let partition_age p =
        let age = ref (BasicSocket.last_time ()) in
        for i = 0 to Array.length  p.part_ages - 1 do
          let a = p.part_ages.(i) in
...
let fixed_partition t name n = 
  Int64Swarmer.partition t name 
    (fun x -> 
      Int64.add x n)
