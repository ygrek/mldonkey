...
let bucket_bit = 20
let bucket_len = 1 lsl bucket_bit
let bucket_mask = bucket_len - 1

  
type 'a t = {
    buckets : 'a array array;
    length : int;
  }

let empty =  { length = 0; buckets = [||] }
  
let unsafe_get t index =
  let bucket_index = index lsr bucket_bit in
  let in_bucket_index = index land bucket_mask in
  Array.unsafe_get (Array.unsafe_get t.buckets bucket_index) in_bucket_index
  
let unsafe_set t index v =
  let bucket_index = index lsr bucket_bit in
  let in_bucket_index = index land bucket_mask in
  Array.unsafe_set (Array.unsafe_get t.buckets bucket_index) in_bucket_index v
  
let get t index =
  let bucket_index = index lsr bucket_bit in
  let in_bucket_index = index land bucket_mask in
  t.buckets.(bucket_index).(in_bucket_index)
  
let set t index v =
  let bucket_index = index lsr bucket_bit in
  let in_bucket_index = index land bucket_mask in
  t.buckets.(bucket_index).(in_bucket_index) <- v

let length t = t.length
  
let make len v =
  let nbuckets = 1 + (len lsr bucket_bit) in
  let last_bucket = len land bucket_mask in
  let buckets = Array.make nbuckets [||] in
  for i = 0 to nbuckets - 2 do
    buckets.(i) <- Array.make bucket_len v
  done;
  buckets.(nbuckets - 1) <- Array.make last_bucket v;
  { 
    length = len;
    buckets = buckets;
  }
...
let copy a =
  ...
  if l = 0 then empty else begin
    let res = make l (unsafe_get a 0) in
    for i = 1 to pred l do
...
let append a1 a2 =
  ...
  if l1 = 0 && l2 = 0 then empty else begin
    let r = make (l1 + l2) (unsafe_get (if l1 > 0 then a1 else a2) 0) in
...
let sub a ofs len =
  if ofs < 0 || len < 0 || ofs > length a - len then invalid_arg "Array.sub"
    ...
    let r = make len (unsafe_get a ofs) in
...
let to_array t =
  let len = length t in
  Array.init len (fun i -> unsafe_get t i)
  
let of_array t =
  let len = Array.length t in
  init len (fun i -> Array.unsafe_get t i)
