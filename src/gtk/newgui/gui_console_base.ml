class box () =
  ...
  in
  let _ =
    GMisc.label ~width:180 ~text:(Gettext.gettext Gui_messages.command)
      ~justify:`LEFT ~line_wrap:true ~xalign:(-1.0) ~yalign:(-1.0)
      ~packing:(_anonymous_container_3#pack ~expand:false ~fill:true) ()
  in
  let we_command =
    GEdit.combo ~popdown_strings:[] ~use_arrows:`DEFAULT ~case_sensitive:true
      ~value_in_list:false ~ok_if_empty:true
  ...
  in
  let _ =
    GMisc.label ~width:180 ~text:(Gettext.gettext Gui_messages.clear_console)
