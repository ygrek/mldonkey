...
let mldonkey_gui_ini = create_options_file 
    (Filename.concat CommonOptions.config_dir "mldonkey_gui2.ini")
  
module KeyOption = struct
    
    let name_to_keysym = 
      ("Button1", Gui2_keys.xk_Pointer_Button1) ::
      ("Button2", Gui2_keys.xk_Pointer_Button2) ::
      ("Button3", Gui2_keys.xk_Pointer_Button3) ::
      ("Button4", Gui2_keys.xk_Pointer_Button4) ::
      ("Button5", Gui2_keys.xk_Pointer_Button5) ::
      Gui2_keys.name_to_keysym
    ...
    let key_to_string (m, k) =
      let s = List.assoc k Gui2_keys.keysym_to_name in
      match m with
        [] -> s
      | _ ->
          let rec iter m s =
