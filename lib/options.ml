...
let define_simple_option
  ...
  let o =
    {option_name = option_name; option_help = option_help;
     ...
     string_wrappers = None; option_hooks = []; option_file = opfile}
  in
  if normalp then
    opfile.file_options <-
...
and parse_option (strm__ : _ Stream.t) =
  ...
  | Some (Kwd "@") ->
      ...
      begin match Stream.peek strm__ with
        Some (Int i) ->
          Stream.junk strm__;
          let v =
            try parse_once_value i strm__ with
              Stream.Failure -> raise (Stream.Error "")
          in
          OnceValue v
      | _ -> raise (Stream.Error "")
  ...
  | Some (Kwd "(") ->
      ...
      List v
  | _ -> raise Stream.Failure
and parse_once_value i (strm__ : _ Stream.t) =
  match Stream.peek strm__ with
    Some (Kwd "@") ->
      Stream.junk strm__;
      begin try Hashtbl.find once_values i with
        Not_found ->
      ...
      try
        ...
        let affect_option o =
          ...
          | e ->
              ...
              List.iter (fun s -> Printf.printf "%s " s) o.option_name;
              print_newline ();
              Printf.printf "  in %s" filename;
              print_newline ();
              Printf.printf "Aborting.";
              print_newline ();
              exit 2
        in
...
let list_to_value name c2v l = List (convert_list name c2v l [])
  
let intmap_to_value name c2v map =
  let list = ref [] in
  Intmap.iter (fun _ v -> list := v :: !list) map;
